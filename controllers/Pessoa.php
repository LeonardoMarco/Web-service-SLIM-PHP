	...
	class Pessoa{
		...
		*/
		public function lista(){
			global $app;
			$sth = $this->PDO->prepare("SELECT * FROM dono");
			$sth->execute();
		...
		*/
		public function get($id){
			global $app;
			$sth = $this->PDO->prepare("SELECT * FROM dono WHERE email = :id");
			$sth ->bindValue(':id',$id);
			$sth->execute();
		...
		*/
		public function nova(){
			...
			*/
			$sth = $this->PDO->prepare("INSERT INTO dono (".implode(',', $keys).") VALUES (:".implode(",:", $keys).")");
		...
		*/
		public function excluir($id){
			global $app;
			$sth = $this->PDO->prepare("DELETE FROM dono WHERE cod_dono = :id");
			$sth ->bindValue(':id',$id);
			$app->render('default.php',["data"=>['status'=>$sth->execute()==1]],200); 
		}
